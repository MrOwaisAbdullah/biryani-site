...
export default function CartContent() {
  ...
  const handleWhatsAppOrder = () => {
    const message = `🛒 New Order:\n\n${cart
      .map(
        (item) =>
          `${item.quantity}x ${item.title} - Rs. ${item.price * (item.quantity || 1)}`
      )
      .join(
        "\n"
    ...
    window.open(
      `https://wa.me/+923371298510?text=${encodeURIComponent(message)}`,
      "_blank"
    );
  };
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="px-4 pb-4 flex-grow">
        ...
        <div className="border-t backdrop-blur-sm mt-8">
          <h3 className="font-semibold text-md sticky top-0 pt-2">
      ...
      <div className="border-t backdrop-blur-sm p-4 space-y-4 mt-auto">
        ...
        <div className="grid gap-2">
          ...
          >
            <Image
              className="absolute left-5 md:left-8 lg:left-16 p-1"
              src={"/whatsapp.png"}
        ...
        <p className="text-center text-sm text-muted-foreground">
          We Only Deliver in Limited Areas
          <br />
          Confirm Before Order!!
          <br />
          <strong>
            <i>Images are only for refference!!!</i>
          </strong>
          <br />
          Thank You!
        </p>
